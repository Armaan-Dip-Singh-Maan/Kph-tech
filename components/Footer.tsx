...
function SocialIcon({ href, children }: { href: string; children: React.ReactNode }) {
  return (
    <a
      ...
      className="flex h-9 w-9 items-center justify-center rounded border border-white/40 text-white transition hover:bg-white/10"
...
export default function Footer() {
  return (
    <footer className="w-full bg-[#1a2b5b] text-white">
      <div className="mx-auto max-w-7xl px-6 py-12 md:px-10 lg:px-14">
        <div className="grid gap-10 md:grid-cols-2 lg:grid-cols-4">
          ...
          <div>
            <h4 className="mb-3 text-sm font-bold uppercase tracking-wide">Services</h4>
            <ul className="space-y-2 text-sm text-white/90">
              {footerServices.map((label) => (
                <li key={label}>
                  <a href="#services" className="transition hover:text-white">{label}</a>
                </li>
              ))}
            </ul>
          </div>
          <div>
            <h4 className="mb-3 text-sm font-bold uppercase tracking-wide">Solutions</h4>
            <ul className="space-y-2 text-sm text-white/90">
              {footerSolutions.map((label) => (
                <li key={label}>
                  <a href="#services" className="transition hover:text-white">{label}</a>
                </li>
              ))}
          ...
          <div>
            <h4 className="mb-3 text-sm font-bold uppercase tracking-wide">Company</h4>
            <ul className="space-y-2 text-sm text-white/90">
              {footerCompany.map((label) => (
                <li key={label}>
                  <a href={label === "Contact" ? "#contact" : "#"} className="transition hover:text-white">{label}</a>
                </li>
              ))}
            </ul>
          </div>
        ...
        <div className="mt-10 border-t border-[#4a5b82] pt-6">
          <div className="flex flex-col items-center justify-between gap-4 text-sm text-white/80 md:flex-row">
            <span>© 2026 KPH Tech. All rights reserved.</span>
